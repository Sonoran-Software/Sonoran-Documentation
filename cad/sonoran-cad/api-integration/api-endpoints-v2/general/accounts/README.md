---
description: v2 general account and identity endpoints.
---

# Accounts

The v2 general account endpoints handle account lookup, permissions, secrets, account links, and account-targeted actions.

## Endpoints

{% content-ref url="apply-permission-key.md" %}
[apply-permission-key.md](apply-permission-key.md)
{% endcontent-ref %}

{% content-ref url="ban-user.md" %}
[ban-user.md](ban-user.md)
{% endcontent-ref %}

{% content-ref url="create-community-link.md" %}
[create-community-link.md](create-community-link.md)
{% endcontent-ref %}

{% content-ref url="check-community-link.md" %}
[check-community-link.md](check-community-link.md)
{% endcontent-ref %}

{% content-ref url="set-community-link.md" %}
[set-community-link.md](set-community-link.md)
{% endcontent-ref %}

{% content-ref url="get-account.md" %}
[get-account.md](get-account.md)
{% endcontent-ref %}

{% content-ref url="get-accounts.md" %}
[get-accounts.md](get-accounts.md)
{% endcontent-ref %}

{% content-ref url="get-permission-catalog.md" %}
[Get Permission Catalog](get-permission-catalog.md)
{% endcontent-ref %}

{% content-ref url="get-account-permissions.md" %}
[Get Account Permissions](get-account-permissions.md)
{% endcontent-ref %}

{% content-ref url="replace-account-permissions.md" %}
[Replace Account Permissions](replace-account-permissions.md)
{% endcontent-ref %}

{% content-ref url="verify-secret.md" %}
[verify-secret.md](verify-secret.md)
{% endcontent-ref %}

{% content-ref url="send-photo.md" %}
[send-photo.md](send-photo.md)
{% endcontent-ref %}
