...
Viewing displays does not require an ACE. Full sync, live cache updates, page scrolling, map panning, and eligible bodycam subscriptions are public and remain subject to server-side validity, distance, routing bucket, interior, cooldown, and rate-limit checks.

The server supplies a permission snapshot to build the menu, then revalidates every player action. A **No permission** entry or locally altered client grants no authority.
...
This group can open the administrative menu and inspect nearby displays, but it cannot place, configure, refresh, diagnose, or delete them. No group grant is needed merely to view an in-world display.
...
This group can open the menu, configure, move, refresh, and diagnose displays but cannot place or delete them.
...
The `forceRefresh` logical action resolves to the `refresh` backend action. The callback runs inside `pcall`. Only boolean `true` allows; errors and other return values deny.
...
Every player receives sanitized display definitions and live board data. There is no viewing ACE. Bodycam image delivery additionally requires an eligible subscription and passes distance, routing bucket, interior, feed, payload, and rate-limit validation.
