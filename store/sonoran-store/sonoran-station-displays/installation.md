---
description: >-
  Install Sonoran Station Displays, SonoranCADFiveM, permissions, and a first
  test display.
tags:
  - fivem
  - installation
...
Viewing displays does not require an ACE. Grant `sonoran.stationdisplay.menu` to each group that should open the administrative menu, then grant only the required action ACEs. See [Permissions](permissions.md) for least-privilege examples and custom permission mode.
