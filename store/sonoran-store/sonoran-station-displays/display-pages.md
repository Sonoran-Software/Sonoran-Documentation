---
description: >-
  Configure display pages, ordering, synchronized runtime changes, and automatic
  rotation.
tags:
  - page rotation
  - active units
...
The server maintains runtime page state for a display. Clients rotate locally from that state, while an eligible nearby manual page change is validated by the server and broadcast to clients. After a manual change, the default hold is 10 seconds before normal rotation resumes.
...
Right and Left Arrow are the default next/previous mappings. The server checks display proximity (with a small network tolerance), routing bucket, interior, page validity, and cooldown.
