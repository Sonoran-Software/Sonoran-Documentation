---
description: >-
  Understand how Sonoran Station Displays reads and classifies SonoranCADFiveM
  unit and call caches.
tags:
  - sonoran cad
  - cache
  - integration
...
The optional Bodycams page also calls `GetBodycamRuntime()` and observes `SonoranCAD::bodycam::RuntimeChanged`. This bridge supplies authoritative active state and player/unit ownership. Station Displays obtains imagery locally through `screenshot-basic`; the bridge does not supply a peer, TURN configuration, relay URL, or hosted image.
...
Bodycam runtime updates are event-driven with a 1-second polling fallback. They are not merged into either call cache. A single 4-second capture loop runs only for active feeds with eligible subscribers.
